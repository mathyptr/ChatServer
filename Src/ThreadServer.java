...
public class ThreadServer extends Thread{
	...
	public int findDest() {
		...
		for(int i=0; i<ThreadVect.size(); i++) {
			if(!((ThreadServer)ThreadVect.elementAt(i)).getClientName().equals(clientName) && ThreadVect.elementAt(i).isAlive())//se la condizione non e' verificata significa che nella posizione i del vettore e' presente l'altro ThreadSever
				pos=i;
		}
			return pos;
	}
	...
	public void SendToOther(String clientData) {
		int dest = -1;
		if ((dest = findDest()) != -1) {// nella successiva istruzione viene inviato il messaggio del client gestito da questo ThreadServer, all'altro ThreadServer presente nell'apposito vettore.
