...
/**
 * Classe per l'implementazione di un Thread Server che si occuperà della
 * comunizione con un client TODO: in questa versione, che prevede da specifica
 * solo due client, durante l'invio di un messaggio viene scelto il primo
 * threadServer diverso da quello utilizzato nel momento in questione. Infatti,
 * essendoci due soli ThreadServer, si tratterà sicuramente dell'altro Thread.
 * Entrambi vegono pero' inseriti in un vettore poiche', implementando un
 * comando per identificare il destinatario, sarà possibile gestire lo scambio
 * di messaggi con un numero superiore di client.
 * 
 * 
...
public class ThreadServer extends Thread {
	...
	public ThreadServer(java.util.Vector<Thread> ThreadVect, Socket clientSocket) {
		this.ThreadVect = ThreadVect; // il vettore contiene tutti i serverThread che gestiscono la communicazione con
										// un client
	...
	public void run() {
		...
		if (clientData.substring(0, 9).equalsIgnoreCase(HELLO_CMD)) { // viene controllato che il client si sia
																		// identificato, mediante il metodo
																		// hello(),prima di iniziare la chat
			clientName = clientData.substring(9, clientData.length());// viene salvato in una variabile il nome del
																		// client
			...
			while (true) {
				...
				if (clientData.equalsIgnoreCase(clientName + ": q!"))
					break;
	...
	/**
	 * metodo per la restituzione del nome del client che comunica con il server in
	 * questione
	 * 
	 ...
	 * implementando un comando per identificare il destinatario sarà possibile
	 * cercare il thread opportuno tramite il nome del Client
	 * 
	...
	public int findDest() {
		...
		for (int i = 0; i < ThreadVect.size(); i++) {
			if (!((ThreadServer) ThreadVect.elementAt(i)).getClientName().equals(clientName)
					&& ThreadVect.elementAt(i).isAlive())// se la condizione non e' verificata significa che nella
															// posizione i del vettore e' presente l'altro ThreadSever
				pos = i;
		}
		return pos;
	}
	...
	/**
	 * metodo per l'invio dei messaggi all'altro client
	 * 
	 * @param clientData String
	 */
	public void SendToOther(String clientData) {
		int dest = -1;
		if ((dest = findDest()) != -1) {// nella successiva istruzione viene inviato il messaggio del client gestito da
										// questo ThreadServer, all'altro ThreadServer presente nell'apposito vettore.
