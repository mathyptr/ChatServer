...
class Chat {
	...
	/**
	 * Metodo per l'avvio del programma in modo interattivo,
	 * mediante richieste dirette all'utente
	 */
	...
	public static void main(String[] args) {//e' possibile passare i parametri al main
		...
		if (args.length >1) //se vengono passati dei parametri dal terminale viene richiamato il metodo che si occupa di leggerli
			cmdLine(args);
		else if( args.length <= 1)	//se non vengono passati i parametri viene richiamato il metodo per la loro richiesta interattiva
			interactive();
